...
app.get('/api/ideas', authenticateToken, async (req, res) => {
    ...
    // Formater les données pour avoir un champ commentsCount simple
    const ideasWithCount = data.map(idea => ({
        ...idea,
        commentsCount: idea.comments && idea.comments[0] ? idea.comments[0].count : 0,
        comments: undefined // On retire la structure complexe renvoyée par Supabase
    }));
